  ...
  L.Control.PolylineMeasure = L.Control.extend({
    ...
    onAdd(map) {
      ...
      if (
        ...
        var title = `${this.options.unitControlTitle.text} [${
    ...
    _changeUnit() {
      ...
      const indexCurrUnit = this.options.unitControlUnits.indexOf(
        this.options.unit
      );
      const indexNextUnit =
      ...
      this._unitControl.title = `${this.options.unitControlTitle.text} [${
        this.options.unitControlTitle[this.options.unit]
      }]`;
    ...
    _computeDistance(line) {
      let totalDistance = 0;
      line.circleCoords.map(
        function (point, point_index) {
          if (point_index >= 1) {
            const distance = line.circleCoords[point_index - 1].distanceTo(
    ...
    _onKeyDown(e) {
      ...
      if (e.keyCode === 27) {
        ...
        if (this._resumeFirstpointFlag === true) {
          ...
          text =
            `${text}<div class="polyline-measure-tooltip-difference">+` +
            `0</div>`;
          text =
          ...
          this._arrPolylines[lineNr].tooltips.map(
            function (item, index) {
              if (index >= 1) {
                const distance = this._arrPolylines[lineNr].circleCoords[
                  index - 1
                ].distanceTo(this._arrPolylines[lineNr].circleCoords[index]);
                const lastCircleCoords =
    ...
    _getDistance(distance) {
      ...
      if (this.options.unit === 'nauticalmiles') {
        ...
        } else if (!this.options.useSubunits) {
    ...
    _polylineArc(_from, _to) {
      function _GCinterpolate(f) {
        ...
        if (diff < 0) {
          lngInterpol -= trunc((diff - 180) / 360) * 360;
        } else {
          lngInterpol -= trunc((diff + 180) / 360) * 360;
        }
    ...
    _updateTooltip(
      ...
      const angleIn = calcAngle(mouseCoords, lastCircleCoords, 'inbound');
      const angleOut = calcAngle(lastCircleCoords, mouseCoords, 'outbound');
      const totalRound = this._getDistance(total);
      const differenceRound = this._getDistance(difference);
      let textCurrent = '';
      ...
      if (this.options.showBearings === true && prevTooltip) {
        const textPrev = prevTooltip._icon.innerHTML;
        const regExp = new RegExp(`${this.options.bearingTextOut}.*°`);
        const textReplace = textPrev.replace(
          regExp,
    ...
    _drawArrow(arcLine) {
      ...
      const center = [P1[0] + diffLat12 / 2, P1[1] + diffLng12 / 2]; // center of Great-circle distance, NOT of the arc on a Mercator map! reason: a) to complicated b) map not always Mercator c) good optical feature to see where real center of distance is not the "virtual" warped arc center due to Mercator projection
      ...
      const iconArrow = L.divIcon({
        ...
        html: `<div style = 'color: ${this.options.arrowColor}; font-size: 16px; line-height: 16px; vertical-align:top; transform: rotate(${cssAngle}deg)'>&#x27a4;</div>`, // best results if iconSize = font-size = line-height and iconAnchor font-size/2 .both values needed to position symbol in center of L.divIcon for all font-sizes.
      });
      const newArrowMarker = L.marker(center, {
        icon: iconArrow,
        zIndexOffset: -50,
    ...
    _mouseMove(e) {
      ...
      const currentTooltip = this._currentLine.tooltips.last();
      const prevTooltip = this._currentLine.tooltips.slice(-2, -1)[0];
      currentTooltip.setLatLng(mouseCoords);
      const distanceSegment = mouseCoords.distanceTo(lastCircleCoords);
      this._updateTooltip(
        currentTooltip,
        prevTooltip,
    ...
    _startLine(clickCoords) {
      ...
      this._currentLine = {
        ...
        handleMarkers(latlng) {
          ...
          const newCircleMarker = new L.CircleMarker(
            latlng,
        ...
        addPoint(mouseCoords) {
          ...
          if (this.circleCoords.length > 1) {
            const arc = polylineState._polylineArc(
              lastCircleCoords,
              mouseCoords
            );
            ...
            const prevTooltip = polylineState._currentLine.tooltips.slice(
              -1,
              -2
            )[0];
          ...
          const tooltipNew = this.getNewToolTip(mouseCoords);
      ...
      const firstTooltip = L.marker(clickCoords, {
        icon,
        interactive: false,
      });
    ...
    _resumePolylinePath(e) {
      ...
      if (e.originalEvent.ctrlKey === true) {
        ...
        const tooltipNew = this._currentLine.getNewToolTip(e.latlng);
    ...
    _clickedArrow(e) {
      if (e.originalEvent.ctrlKey) {
        const lineNr = e.target.cntLine;
        const arrowNr = e.target.cntArrow;
        ...
        const arc1 = this._polylineArc(
          this._arrPolylines[lineNr].circleCoords[arrowNr],
          e.latlng
        );
        arc1.pop();
        const arc2 = this._polylineArc(
          e.latlng,
    ...
    _dragCircleMousemove(e2) {
      ...
      const currentCircleCoords = L.latLng(
      ...
      if (circleNr >= 1) {
        ...
        const newLineSegment1 = this._polylineArc(
      ...
      if (circleNr < this._arrPolylines[lineNr].circleCoords.length - 1) {
        ...
        const newLineSegment2 = this._polylineArc(
          currentCircleCoords,
          this._arrPolylines[lineNr].circleCoords[circleNr + 1]
        );
        Array.prototype.splice.apply(
      ...
      this._arrPolylines[lineNr].tooltips.map(
        function (item, index) {
          if (index >= 1) {
            ...
            const lastCircleCoords =
              this._arrPolylines[lineNr].circleCoords[index - 1];
            const mouseCoords = this._arrPolylines[lineNr].circleCoords[index];
            totalDistance += distance;
            this._arrPolylines[lineNr].distance = totalDistance;
            const prevTooltip = this._arrPolylines[lineNr].tooltips[index - 1];
            this._updateTooltip(
              item,
              prevTooltip,
              totalDistance,
    ...
    _resumeFirstpointMousemove(e) {
      ...
      let totalDistance = 0;
      const distance = mouseCoords.distanceTo(
        this._arrPolylines[lineNr].circleCoords[0]
      );
      const lastCircleCoords = mouseCoords;
      const currentCoords = this._arrPolylines[lineNr].circleCoords[0];
      totalDistance += distance;
      const prevTooltip = this._tooltipNew;
      const currentTooltip = this._arrPolylines[lineNr].tooltips[0];
      this._updateTooltip(
        currentTooltip,
        prevTooltip,
        totalDistance,
    ...
    _resumeFirstpointClick(e) {
      ...
      const arc = this._polylineArc(e.latlng, currentCircleCoords);
      const arrowMarker = this._drawArrow(arc);
    ...
    _dragCircle(e1) {
      ...
      if (e1.originalEvent.ctrlKey) {
        ...
        if (e1.target.cntCircle === 0) {
          ...
          var text = '';
          if (this.options.showBearings === true) {
            text = `${text + this.options.bearingTextIn}:---°<br>${
          ...
          this._currentLine.tooltips.map(
            function (item, index, arr) {
              if (index >= 1) {
                let distance;
                let mouseCoords;
                const prevTooltip = this._currentLine.tooltips[index - 1];
                const lastCircleCoords =
