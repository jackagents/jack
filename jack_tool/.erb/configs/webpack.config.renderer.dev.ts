...
// const clients = ['iwd', 'iba', 'tewa'];
/**
 * Client argument from command line
 */
// const client = clients.includes(process.argv[process.argv.length - 1])
...
const configuration: webpack.Configuration = {
  ...
  entry: [
    ...
    path.join(webpackPaths.entryPointsPath, 'index_editor.tsx'),
  ],
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
      ...
      template: path.join(webpackPaths.entryPointsPath, 'index.ejs'),
      minify: {
  ...
  devServer: {
    ...
    setupMiddlewares(middlewares) {
      ...
      // let args = ['run', 'start:main', '--env', client];
