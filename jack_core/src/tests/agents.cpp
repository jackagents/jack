    ...
    jack::AgentHandle agentHandle =
        bdi.agent("TestAgent1")
            ...
            .handleAction("Plan1Action",
                          [&](jack::Agent &agent, jack::Message &,
                              jack::Message &, jack::ActionHandle) {
...
TEST_F(AgentsTest, AgentPlanSwitch)
{
    ...
    // 1. Create 1 goal, "Goal" with 2 candidate plans, "PlanA" and "PlanB".
    const std::string SWITCH_PLANS = "switchPlans";
    jack::MessageBuilder beliefs   = bdi.message("Beliefs")
                                        .field<bool>(SWITCH_PLANS)
                                        .commit();
    ...
    jack::PlanBuilder   planA   = bdi.plan("PlanA")
        .pre([&](const jack::BeliefContext &context) -> bool {
            std::shared_ptr<jack::Message> beliefset = context.message(beliefs.name());
            EXPECT_NE(beliefset, nullptr);
            EXPECT_NE(beliefset->getField(SWITCH_PLANS), nullptr);

            auto switchPlans = *beliefset->getPtr<bool>(SWITCH_PLANS);
            return switchPlans == false;
        })
        .handles(goal)
        .body(bdi.coroutine()
                 .action(actionA.name()))
    ...
    jack::PlanBuilder planB     = bdi.plan("PlanB")
        .pre([&](const jack::BeliefContext &context) -> bool {
            ...
            bool switchPlans = *beliefset->getPtr<bool>(SWITCH_PLANS);
            return switchPlans == true;
        })
        .handles(goal)
        .body(bdi.coroutine()
                 .action(actionB.name()))
    ...
    bool didWeSwitch = false; // Indicates if we switched to PlanB at any point
        ...
        bdi.agent("AgentTemplate")
            ...
            .handleAction(actionA.name(),
                          [&](jack::Agent &agent, jack::Message &, jack::Message &out, jack::ActionHandle) {
                              ...
                              // \note Updating a belief/percept will set a dirty flag
                              // (m_scheduleDirty) and trigger the scheduler to replan, i.e. we will
                              // use it to switch to PlanB using plan pre-conditions that check that
                              // belief.

                              jack::Message beliefset = *agent.context().message(beliefs.name());
                              beliefset.setFieldValue(SWITCH_PLANS, true);
                              agent.sendMessage(std::move(beliefset), false /*broadcastToBus*/);

                              // \note We halt the action from completing immediately by returning
                              // PENDING. We should transition to PlanB because we set the belief
                              // above
                              return jack::Event::PENDING;
                          })
            .handleAction(actionB.name(), [&](jack::Agent &, jack::Message &, jack::Message &, jack::ActionHandle) {
                didWeSwitch = true;
                return jack::Event::SUCCESS;
            })
            .commitAsAgent();
...
TEST_F(AgentsTest, AppliedEffectsShouldNotSendPerceptEvent)
{
    ...
    jack::GoalBuilder goal =
        bdi.goal("Goal")
            .satisfied([&](const jack::BeliefContext &context) -> bool { return effectsApplied; })
            .commit();
    ...
    jack::AgentHandle agentHandle =
        bdi.agent("AgentWithBatteryTemplate")
            ...
            .handleAction(
                action.name(),
                [&](jack::Agent &agent, jack::Message &msg, jack::Message &out, jack::ActionHandle) { return jack::Event::SUCCESS; })
            .commitAsAgent()
...
TEST_F(AgentsTest, DISABLED_DoubleSchedulingOfFinishedDesire)
{
    ...
    /// will create 2 events that are handled in the same tick.
    {
        ...
        std::shared_ptr<jack::Message> beliefSet = agent->message(beliefs.name());
    ...
    auto agentBuilder = bdi.agent("AgentTemplate")
                           ...
                           .handleAction(action.name(),
                                         [&](jack::Agent &agent, jack::Message &msg, jack::Message &out, jack::ActionHandle handle) {
                                             ...
                                             jack::Message msgToSend = agent.engine().createMessage(pingMsg.name());
                                             agentA->sendMessageToHandler(std::move(msgToSend));

                                             agentBSentMsg = true;
                                             return jack::Event::SUCCESS;
                                         })
                           .handleMessage(pingMsg.name(), [&](jack::Agent &agent, const jack::Message& message) {
...
TEST(AgentTest, AttachedServices)
{
    ...
    std::string_view MSG_FIELD_NAME = "flag";
    jack::MessageBuilder msg        = bdi.message("Message")
                                         .field<bool>(MSG_FIELD_NAME)
                                         .commit();
    ...
    std::shared_ptr<jack::Message> agentMsg = agent->message(msg.name());
    EXPECT_FALSE(*agentMsg->getConstPtr<bool>(MSG_FIELD_NAME));

    /// Send the message through the service
    jack::Message msgToSend = bdi.createMessage(msg.name());
    msgToSend.setFieldValue(MSG_FIELD_NAME, true);
    service->sendMessage(std::move(msgToSend), false /*broadcastToBus*/, nullptr /*recipient*/);
    ...
    EXPECT_TRUE(*agentMsg->getConstPtr<bool>(MSG_FIELD_NAME));
}
