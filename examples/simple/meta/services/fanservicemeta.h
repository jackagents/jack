#ifndef SIMPLE_FAN_SERVICE_META_H
#define SIMPLE_FAN_SERVICE_META_H

/// JACK
#include <jack/service.h>

/// Third Party
#include <string_view>

namespace aos::jack { class Engine; }


class FanControl;
class FanStatus;


class FanService;
/******************************************************************************
 * \class  FanServiceMeta
 * \author jackmake
 * \source simple.jack
 ******************************************************************************/
class FanServiceMeta : public aos::jack::Service
{
public:
    /**************************************************************************
     * Constructor/Destructors
     **************************************************************************/
    FanServiceMeta(aos::jack::Engine& bdi, std::string_view name);
    FanServiceMeta(const FanServiceMeta* other, std::string_view name);
    virtual ~FanServiceMeta() { }

    FanServiceMeta(const FanServiceMeta& other) = delete;
    FanServiceMeta& operator=(const FanServiceMeta& other) = delete;

    /**************************************************************************
     * Functions
     **************************************************************************/
    aos::jack::Service* clone(std::string_view name) const override;

    /// \note Actions
    virtual aos::jack::Event::Status onTurnFanOn(const FanControl& request, aos::jack::ActionHandle handle) = 0;
    virtual aos::jack::Event::Status onTurnFanOff(const FanControl& request, aos::jack::ActionHandle handle) = 0;

protected:
    /// Link the action handler to this service's derived action handler.
    void setupHandlers();

public:
    /**************************************************************************
     * Static Functions
     **************************************************************************/
    /// Create an instance of the service and return the service handle
    /// @param proxy Create a proxy service that will forward events onto the bus to the real instance
    static aos::jack::ServiceHandle create(aos::jack::Engine& bdi, std::string_view name, bool proxy, const aos::jack::UniqueId& uuid = aos::jack::UniqueId::random());

    /// Create an instance of the service and return a pointer to the service
    /// @param proxy Create a proxy service that will forward events onto the bus to the real instance
    static FanService* createInstance(aos::jack::Engine& bdi, std::string_view name, bool proxy, const aos::jack::UniqueId& uuid = aos::jack::UniqueId::random());

    /**************************************************************************
     * Static Fields
     **************************************************************************/
    /// The name of the C++ class
    static constexpr inline std::string_view CLASS_NAME = "FanService";

    /// The name of this object in the JACK model
    static constexpr inline std::string_view MODEL_NAME = "simple.Fan Service Template";

    /// \todo: This should use the qualified name so that the editor can
    /// use nice names. But its been @harcoded to the bumpy case.
    /// The name of this object in the simulator's ECS
    static constexpr inline std::string_view COMPONENT_NAME = "simple.Fan Service Component";
};

#endif /// SIMPLE_FAN_SERVICE_META_H